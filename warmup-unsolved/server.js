MY NOTES ARE CONFUSING USE THE SOLVED FILE TO REPLACE THIS ONEEEEEEEEEE
...
app.get("/all", function(req, res) {
  ...
  db.books.save({}, function(error, saved) {
...
app.get("/read", function(req, res) {
  db.books.find({}, function(error, saved) {
    // Log any errors if the server encounters one
    if (error) {
      console.log(error);
    }
  ...
  db.books.unread().sort({ name: 1 }, function(error, found) {
    // Log any errors if the server encounters one
    if (error) {
      console.log(error);
    }
